...
;; remember that matrix co-ords go row then column, unlike how we 
;; are used to. The axies (as it were) are generally named m & n
;; where m is rows and n is columns

;; matrices will be stored as 1D arrays with 9 elements, this is for
;; speed reasons

;; taken from Ogre, I'm assuming this is some kind of lower float 
;; boundary
;; const Real Matrix3::ms_fSvdEpsilon = 1e-04;

;; used for single value decomposition...need to real up on this again
...
(defun matrix-row (matrix row-num)
  ...
  (let ((offset (* row-num 3)))
    (vector (svref matrix offset)
	    (svref matrix (+ 1 offset))
	    (svref matrix (+ 2 offset)))))

(defun matrix-column (matrix col-num)
  (assert (and (>= col-num 0) (< col-num 3)))
  (vector (svref matrix col-num)
	  (svref matrix (+ 3 col-num))
	  (svref matrix (+ 6 col-num))))

(defun matrix-element (matrix row col)
  (svref matrix (+ col (* row 3))))
...
(defun c-= (matrix-a matrix-b)
  (loop for i 
     below 9
     if (/= (svref matrix-a i) (svref matrix-b i))
     do (return nil)
     finally (return t)))
...
(defun c-+ (matrix-a matrix-b)
  (let ((r (make-matrix3)))
    (loop for i below 9
	 do (setf (svref r i) (+ (svref matrix-a i) 
				 (svref matrix-b i))))
    r))
...
;; 	      do (setf (svref result)
;; 		       (+ (matrix-element matrix-a 0 col)))))
;;     result))
