...
(defmethod glut:display ((win arc-tut-window))
  (adjust-vertex-data win (compute-position-offsets))
...
(defun compute-position-offsets ()
  (let* ((loop-duration 5.0)
	 ...
	 (curr-time-through-loop (mod elapsed-time 
				      loop-duration))
	 (x (* 0.005 (cos (* scale curr-time-through-loop))))
	 (y (* 0.005 (sin (* scale curr-time-through-loop)))))
    (make-vector2 x y)))
