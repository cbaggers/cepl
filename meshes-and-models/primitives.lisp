...
(defun box-data (&key (width 1.0) (height 1.0) (depth 1.0)
                   (normals t) (tex-coords t) (inverse-normals nil))
  (let ((width (/ width 2.0))
        (height (/ height 2.0))
        (depth (/ depth 2.0)))
    (list (if (not (or normals tex-coords))
              (list  (v! (- width) (- height) depth)
                     ...
                     (v! (- width) (- height) depth))
              (list  `(,(v! (- width) (- height) depth)
                        ,@(when normals `(,(v! 0.0 0.0 (if inverse-normals
                                                           -1.0 1.0))))
                        ,@(when tex-coords `(,(v! 0.0 1.0))))
                     `(,(v! width (- height) depth)
                        ,@(when normals `(,(v! 0.0 0.0 (if inverse-normals
                     ...
                     `(,(v! width height (- depth))
                        ...
                        ,@(when tex-coords `(,(v! 0.0 0.0))))
                     `(,(v! (- width) (- height) (- depth))
                        ,@(when normals `(,(v! (if inverse-normals 1.0 -1.0)
                                                0.0 0.0)))
                        ,@(when tex-coords `(,(v! 0.0 1.0))))
                     `(,(v! (- width) (- height) depth)
                        ,@(when normals `(,(v! (if inverse-normals 1.0 -1.0)
                     ...
                     `(,(v! width height depth)
                        ...
                        ,@(when tex-coords `(,(v! 0.0 0.0))))
                     `(,(v! (- width) height depth)
                        ,@(when normals `(,(v! 0.0 (if inverse-normals -1.0 1.0)
                                                 0.0)))
                        ,@(when tex-coords `(,(v! 0.0 1.0))))
                     `(,(v! width height depth)
                        ,@(when normals `(,(v! 0.0 (if inverse-normals -1.0 1.0)
