...
(defun load-model (filename)
  (destructuring-bind ((verts indicies)) (model-parsers:load-obj-file filename)
    (let ((gstream (make-vertex-stream verts :index-array indicies)))
...
(defun init () 
  ...
  (setf *wibble* (load-model "./wibble.3ds")))
