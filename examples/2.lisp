...
(cgl:defprogram prog-2 ((position :vec4) &uniform (offset :vec2))
  (:vertex (setf gl-position (+ position (vec4 (x offset) (y offset) 0.0 0.0))))
  (:fragment (out output-color (vec4 1.0 0.0 0.0 1.0))))
...
(defun draw (gstream)
  ...
  (cffi-sys:with-pointer-to-vector-data 
      (ptr (v! (* 0.5 (sin *loop-pos*)) (* 0.5 (cos *loop-pos*))))
    (prog-2 gstream :offset ptr))
  (gl:flush)
  (sdl:update-display))
