...
(defmethod pull1-g ((c-arr c-array))
  (let* ((elem-type (c-array-element-type c-arr))
         (dims (c-array-dimensions c-arr))
         (lisp-type (or (cepl.types::lisp-equivalent-of-keyword-cffi-type
                         elem-type)
                        elem-type))
         (arr (make-array dims :element-type lisp-type))
         (from (get-typed-from-foreign elem-type)))
    (labels ((1d (ptr x)
               (setf (aref arr x) (funcall from ptr)))
             (2d (ptr x y)
               (setf (aref arr x y) (funcall from ptr)))
             (3d (ptr x y z)
               (setf (aref arr x y z) (funcall from ptr)))
             (4d (ptr x y z w)
               (setf (aref arr x y z w) (funcall from ptr))))
      (across-c-ptr (ecase= (length dims)
                      (1 #'1d)
                      (2 #'2d)
                      (3 #'3d)
                      (4 #'4d))
