...
(defn map-c-into ((destination-c-array (or null c-array)) (function function)
...
(defn %across-c-2d ((func function) (arr c-array))
  ...
  (declare (optimize (speed 3) (safety 0) (debug 1)))
  (let ((dim (c-array-dimensions arr)))
    (loop :for y :below (the c-array-index (first dim)) :do
       (loop :for x :below (the c-array-index (second dim)) :do
...
(defn %across-c-3d ((func function) (arr c-array))
  ...
  (let ((dim (c-array-dimensions arr)))
    (loop :for z :below (the c-array-index (first dim)) :do
       (loop :for y :below (the c-array-index (second dim)) :do
          (loop :for x :below (the c-array-index (third dim)) :do
...
(defn %across-c-4d ((func function) (arr c-array))
  ...
  (declare (optimize (speed 3) (safety 1) (debug 1)))
  (let ((dim (c-array-dimensions arr)))
    (loop :for w :below (the c-array-index (first dim)) :do
       (loop :for z :below (the c-array-index (second dim)) :do
          (loop :for y :below (the c-array-index (third dim)) :do
             (loop :for x :below (the c-array-index (fourth dim)) :do
  ...
  (let ((dim (c-array-dimensions arr)))
    (loop :for w :below (the c-array-index (first dim)) :do
       (loop :for z :below (the c-array-index (second dim)) :do
          (loop :for y :below (the c-array-index (third dim)) :do
             (loop :for x :below (the c-array-index (fourth dim)) :do
                (let ((ptr (ptr-index arr w z y x)))
                  (funcall func ptr w z y x)))))))
  arr)
