...
(defun map-into-c (destination-c-array function source-c-array)
  (let ((func function)
    ...
    (case= (c-array-rank src)
      (1 (%map-into-c-1d dst func src))
      (2 (%map-into-c-2d dst func src))
      (3 (%map-into-c-3d dst func src))
      (4 (%map-into-c-4d dst func src)))))

(defun %map-into-c-1d (dst func src)
  (let ((dim (c-array-dimensions src)))
    (loop :for i :below (first dim) :do
       (setf (aref-c dst i) (funcall func (aref-c src i)))))
  dst)

(defun %map-into-c-2d (dst func src)
  (let ((dim (c-array-dimensions src)))
...
(defun %map-into-c-3d (dst func src)
  (let ((dim (c-array-dimensions src)))
...
(defun %map-into-c-4d (dst func src)
  (let ((dim (c-array-dimensions src)))
