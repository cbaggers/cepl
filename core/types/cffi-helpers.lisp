...
(defmacro make-typed-from-foreign ()
  (let ((type-equivalents
        ...
        (types (append `((:int8 0 nil nil)
                       ...
                       %cepl.types::*extra-primitive-types*)))
    `(progn
       ,@(loop
            :for (type len nil comp-lisp-type) :in types
            ...
            :for lisp-val-type :=
              (or (assocr type type-equivalents) t)
            :append
              `((defn-inline ,from ((ptr cffi:foreign-pointer))
                    ,lisp-val-type
                  (declare (optimize (speed 3) (safety 1) (debug 0)))
                  (mem-aref ptr ,type))
                (defn-inline ,to ((ptr cffi:foreign-pointer)
                                  (value ,lisp-val-type))
                    ,lisp-val-type
                  (declare (optimize (speed 3) (safety 1) (debug 0)))
