...
(defun2 assert-valid-gpipe-form (pipeline-name gpipe-args)
...
(defun2 assert-valid-gpipe-shader-implicit-form (pipeline-name gpipe-args)
...
(defun2 assert-valid-stage-specs (names)
...
(defun2 find-invalid-defpipeline-options (options valid-keys)
  (labels ((check (x) (member (car x) valid-keys)))
    (remove-if #'check options)))

(defun2 function-formp (x)
  (and (listp x)
       (eq (first x) 'function)
       (= (length x) 2)
       (or (symbolp (second x))
