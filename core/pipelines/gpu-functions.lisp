...
(defun2 defun-g-common (name args body equiv)
...
(defun2 assert-arg-format (gfunc-name x)
  (unless (listp x)
...
(defun2 %def-gpu-function (name in-args uniforms body instancing
                          doc-string equiv context)
...
(defun2 %update-gpu-function-data (spec depends-on compiled)
...
(defun2 %update-glsl-stage-data (spec)
  "[0] Add or update the spec"
  (setf (gpu-func-spec spec) spec));;[0]
...
(defun2 make-stand-in-lisp-func (spec)
...
(defun2 %aggregate-uniforms (uniforms &optional accum)
...
(defun2 aggregate-uniforms (keys &optional accum interal-uniforms-p)
...
(defun2 get-func-as-stage-code (stage)
  (with-gpu-func-spec stage
    (list in-args uniforms context body)))

;;--------------------------------------------------

(defun2 %varjo-compile-as-pipeline (draw-mode parsed-gpipe-args)
...
(defun2 parsed-gpipe-args->v-translate-args (draw-mode stage-pair
...
(defun2 get-possible-designators-for-name (name)
  (mapcar λ(with-gpu-func-spec _
             (cons name (mapcar #'second in-args)))
          (gpu-func-specs name)))

(defun2 get-stage-key (stage-designator &optional (error-on-symbol t))
  (cond
...
(defun2 validate-stage-names (names)
  (let* (invalid
         (valid
...
(defun2 try-injecting-a-constant (constant-name)
...
(defun2 try-guessing-a-varjo-type-for-symbol (s)
...
(defun2 guess-a-varjo-bool-type (x)
  (if (eql x t)
      :bool
      (error "Cant guess a suitable type for ~s" x)))

(defun2 guess-a-varjo-array-type (x)
  (typecase x
...
(defun2 guess-a-varjo-number-type (x)
  (typecase x
...
(defun2 guess-a-varjo-float-type (x)
  (if (typep x 'single-float)
      :float
      :double))

(defun2 guess-a-varjo-integer-type (x)
  (typecase x
    ((signed-byte 32) :int)
...
(defun2 interactive-delete-gpu-function (name)
  (let ((picked
